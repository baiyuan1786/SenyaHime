...
#               @hostIP
#               @accessToken 
# Notice:       提供一个简单的Flask服务器接口来处理QQ机器人请求
###############################################################################
class shamrockbot:
    '''提供一个简单的Flask服务器接口来处理QQ机器人请求'''
    def __init__(self, name: str, clientPort: int, hostPort: int, hostIP: str, accessToken: str) -> None:
        ...
        self.baseURL = None
        #fr"http://{self.clientIP}:{self.clientPort}" 
    ...
    # Notice:       
    ###############################################################################
    def URLupdate(self, clientIP: str):
        '''更新URL'''
        self.baseURL = fr"http://{clientIP}:{self.clientPort}" # baseURL只有IP地址和端口信息

    ###############################################################################
    # Function:     HTTPcallbackDef
    # Input:        void     
    # Notice:       
    ###############################################################################
    def HTTPcallbackDef(self, callbackFunc):
