...
###############################################################################
def imageToBase64(image_path: str, upDown: Literal["up", "down"]) -> str:
    '''将本地的一张图片转换为base64格式'''
    ...
    if upDown == "up":
        ...
        raise ValueError(f"[imageToBase64]upsupport updown \"{upDown}\"")
...
class image(messageSegment):
    '''图片类型消息'''
    def __init__(self, file: str | None, upDown: Literal["up", "down"] = "up", url: str | None = None, type: str | None = None, subType: int | None = None):
        """图片类型消息
        ...
        if os.path.exists(file):
            file = imageToBase64(image_path = file, upDown = upDown)
