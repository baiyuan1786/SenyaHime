...
from control import PRODUCTION_MODE
...
###############################################################################
class httpserver:
    '''使用http协议的shamrockbot服务器类'''
    def __init__(self, name: str, clientPort: int, hostPort: int, clientIP: str, hostIP: str, \
    ...
    def baseURL(self):
        '''获取基础链接'''
        # 使用自动IP和port
        clientIP = self.clientIP if self.clientIP != "auto" else request.remote_addr
        clientPort = self.clientPort if self.clientPort != "auto" else request.environ.get('REMOTE_PORT')

        baseURL = fr"http://{clientIP}:{clientPort}" # baseURL只有IP地址和端口信息
        return baseURL
    ...
    def callbackDef(self, callbackFunc):
        '''为当前机器人定义HTTP回调函数,重复调用此函数将增加新的规则'''
        self.app.add_url_rule('/', 'post_data', callbackFunc, methods=['POST'])
    ...
    def run(self):
        ...
        logInfo(f"[{self.name}]Running on http://{self.hostIP}:{self.hostPort}")
        if PRODUCTION_MODE:
            serve(self.app, host = self.hostIP, port = self.hostPort)
        else:
            self.app.run(debug = True, port = self.hostPort, host = self.hostIP)
